...
exports.HostsStateManager = void 0;
...
class HostsStateManager {
    constructor() {
        if (process.platform == 'darwin') {
            // Mac does not natively have ssh-askpass or have an X11 DISPLAY, so we
            // need to fake it
            this.sshEnv = Object.assign(Object.assign({}, process.env), { "DISPLAY": "1", "SSH_ASKPASS": path.join(__dirname, "askpass.osascript") });
            ...
            this.sshEnv = Object.assign({}, process.env);
        }
    ...
    pushToFrontend() {
        (0, main_1.sendToFrontend)('updateHostsState', this.hostsState);
    }
    getHostsState() {
    ...
    getRemotePorts(hostName) {
        ...
        // spawn ssh session
        const spawned = (0, child_process_1.exec)(sshCommand, { "env": this.sshEnv, "timeout": 20000 }, (err, stdout, stderr) => {
            let output = '' + stdout;
            ...
            for (var i = 1; i < rows.length; i++) {
                ...
                if (parseInt(port) > 19999)
                    continue; // Skip High Port Numbers
                if (portsUsed.indexOf(port) != -1)
                    continue; // Skip duplicate ports (due to ipv4 and ipv6)
                portsUsed.push(port);
                const entry = {
            ...
            for (var i = 0; i < ns_rows.length; i++) {
                ...
                if ((portInt < 1000 || portInt > 9999) && portInt != 80 && portInt != 443)
                    continue; // Skip high/low port numbers except http(s)
            ...
            if (refHost['remoteProcesses'].length) {
                newProcessList = ProcessManager_1.ProcessManager.instance.mergeProcessLists(refHost['remoteProcesses'], processList);
            }
            else {
    ...
    hostsStateFromConfig() {
        const hosts = SSHConfigManager_1.SSHConfigManager.instance.getHosts();
...
exports.HostsStateManager = HostsStateManager;
//# sourceMappingURL=HostsStateManager.js.map